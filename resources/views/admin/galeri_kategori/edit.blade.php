...
<head>
    @include('admin.head')
    <title>Edit Kategori Galeri: {{ $kategoriGaleri->nama_kategori }} - Admin Panel</title>
        ...
        <div class="body-wrapper">
            ...
            <div class="container-fluid">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Edit Kategori Galeri: <span class="text-primary">{{ $kategoriGaleri->nama_kategori }}</span></h1>
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Formulir Edit Kategori Galeri</h6>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.galeri-kategori.update', $kategoriGaleri->slug) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="mb-3">
                                <label for="nama_kategori" class="form-label">Nama Kategori <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('nama_kategori') is-invalid @enderror" id="nama_kategori" name="nama_kategori" value="{{ old('nama_kategori', $kategoriGaleri->nama_kategori) }}" required autofocus>
                                @error('nama_kategori')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $kategoriGaleri->slug) }}" placeholder="Contoh: welpart-2024">
                                <small class="form-text text-muted">Jika dikosongkan dan nama kategori diubah, slug akan dibuat otomatis. Isi jika ingin slug kustom.</small>
                                @error('slug')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $kategoriGaleri->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
